...
def spectra_plot(df ,
    ...
    ax1.set_xlabel(x_title, fontdict=dict(size=28, fontname="Times New Roman", fontweight='bold'))
    ax1.set_ylabel(y_title, fontdict=dict(size=28, fontname="Times New Roman", fontweight='bold'))
    ...
    if graph_item != "none":
        ax1.text(1490, 0.21, s= graph_item, fontdict=dict(size=25, fontname="Times New Roman", fontweight="bold", style="italic"))

    if save_fig:
        fig.savefig(file_name + texts_on_graph_dict[0]["text_sentence"] + ".tiff", dpi=600, bbox_inches='tight',pil_kwargs={"compression": "tiff_lzw"})